...
fn main() {
    ...
    for p in METADATA.peripherals {
        if let Some(regs) = &p.registers {
            for pin in p.pins {
                ...
                // ADC is special
                // The local F071 milestone proves inventory only. Its ADC HAL
                // constants have not yet been independently evidenced.
                if regs.kind == "adc" && !chip_name.starts_with("py32f071") {
                    if p.rcc.is_none() {
                        continue;
                    }
                ...
                // DAC has no HAL driver or impl_dac_pin macro yet. Keep the
                // peripheral singleton in the inventory without generating an
                // unusable driver binding.
            }
        }
    }
    ...
    fn get_chip_cfgs(chip_name: &str) -> Vec<String> {
        ...
        if chip_name.starts_with("py32f002a") || chip_name.starts_with("py32f002b") {
