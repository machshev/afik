...
pub enum Key { Menu, Up, Down, Exit, Star, Function, Digit(u8) }
...
pub fn scan<I: MatrixIo>(io: &mut I) -> Option<Key> {
    ...
    for row in 0..ROWS.len() {
        io.select_row(row);
        let Some(columns) = stable_columns(io) else { result = None; break };
        let held = !columns & 0x0f;
        if held == 0 { continue; }
        if held.count_ones() != 1 || result.is_some() { result = None; break; }
        result = Some(MATRIX[row][held.trailing_zeros() as usize]);
    }
...
fn stable_columns<I: MatrixIo>(io: &mut I) -> Option<u8> {
    ...
    for _ in 0..8 {
        ...
        if current == previous {
            matches += 1;
            if matches == 3 { return Some(current); }
        } else { previous = current; matches = 0; }
    }
    None
}
...
impl MatrixIo for K5Matrix {
    fn select_row(&mut self, row: usize) {
        for pin in ROWS { gpio::write_pin(Port::A, pin, true); }
    ...
    fn settle(&mut self) { for _ in 0..48 { core::hint::spin_loop(); } }
    fn restore_shared_pins(&mut self) {
...
mod tests {
    ...
    struct Fake { held: Option<(usize, usize)>, row: usize, restored: bool }
    impl MatrixIo for Fake {
        fn select_row(&mut self, row: usize) { self.row = row; }
        fn columns(&mut self) -> u8 { self.held.filter(|(r, _)| *r == self.row).map_or(0x0f, |(_, c)| 0x0f & !(1 << c)) }
        fn settle(&mut self) {}
        fn restore_shared_pins(&mut self) { self.restored = true; }
    }
    #[test]
    fn maps_all_positions_and_restores_pins() {
        let expected = [Key::Menu, Key::Digit(1), Key::Digit(4), Key::Digit(7), Key::Up, Key::Digit(2), Key::Digit(5), Key::Digit(8), Key::Down, Key::Digit(3), Key::Digit(6), Key::Digit(9), Key::Exit, Key::Star, Key::Digit(0), Key::Function];
        for (index, key) in expected.into_iter().enumerate() {
            let mut io = Fake { held: Some((index / 4, index % 4)), row: 0, restored: false };
            assert_eq!(scan(&mut io), Some(key)); assert!(io.restored);
        }
    }
    #[test]
    fn no_key_restores_pins() {
        let mut io = Fake { held: None, row: 0, restored: false };
        assert_eq!(scan(&mut io), None); assert!(io.restored);
    }
}
