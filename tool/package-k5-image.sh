...
elf_path="${1:-target/thumbv6m-none-eabi/release/radio-firmware-k5}"
raw_path="${2:-target/thumbv6m-none-eabi/release/afik-k5-v1.raw}"
...
tool/verify-k5-image.sh "$elf_path"

temporary_directory="$(mktemp -d)"
trap 'rm -rf "$temporary_directory"' EXIT
temporary_raw="$temporary_directory/k5-v1.raw"

# The qualified bootloader writes a fixed 240 pages, so the file is exactly the
# application region and every byte AFIK does not use is left erased.
llvm-objcopy \
